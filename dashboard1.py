...
from alegorithm1 import server
###############################

app = dash.Dash(__name__, server=server, url_base_pathname='/beer_dash')
...
def grab_similar_beers(df, index, neighbs = 100):
    new_data_point = np.append(buildDescVector(df.loc[index, 'DescriptionTokenized'], vector_dim),[df.loc[index, 'abv'],df.loc[index, 'ibu'],df.loc[index, 'rating'], df.loc[index, 'style_enc']]).reshape([-1,304])
